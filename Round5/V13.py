...
# In V13 these are no longer suppressed: they're routed to PennyDisableTrader
# (4 plain disabled products) and PebblesBasketBalancer (the 2 disabled
# PEBBLES) so every product in the universe has a quoting path.
# The set is kept defined (and empty) so that downstream code referencing it
# still works without conditional logic.
...
# ---------------------------------------------------------------------------
# V13 ADDITIONS -- coverage layer for previously disabled products
# ---------------------------------------------------------------------------
# Two lightweight quoters that handle the 6 products that V12 explicitly
# disabled because the macro / V5 engines lost money on them. The goal here
# is universe coverage (no product left untraded), not aggressive PnL
# generation. Hence: passive-only quoting, conservative size, and a
# basket-aware variant for the two disabled PEBBLES.

# Products the V12 structural study flagged as poor MM fits.
# Split into two groups: pure penny-MM and PEBBLES basket balancers.
DISABLE_PENNY_PRODUCTS: tuple[str, ...] = (
    "PANEL_4X4",
    "ROBOT_MOPPING",
    "ROBOT_VACUUMING",
    "SLEEP_POD_LAMB_WOOL",
)
DISABLE_BASKET_PRODUCTS: tuple[str, ...] = (
    "PEBBLES_M",
    "PEBBLES_XS",
)
DISABLE_PENNY_SIZE = PENNY_SIZE  # mirror PENNY_SIZE so the helper can quote at parity

# Residual threshold: only act on disabled PEBBLES when the basket is
# meaningfully off its 50,000 invariant. The active V5 PEBBLES already
# trigger at 8.5; we set this slightly higher so the disabled members only
# nudge the basket when the active arb hasn't closed the gap by itself.
DISABLE_BASKET_TRIGGER = 10.0


class PennyDisableTrader:
    """
    Conservative penny-market-maker for the 4 plain disabled products.

    For each product:
      - if the spread is >= 2 ticks, quote at bid+1 and ask-1
      - if the spread is 1 tick, quote at BBO (no improvement possible)
      - never cross our own quote, never aggress the book
      - size capped to DISABLE_PENNY_SIZE on each side, also gated by
        the remaining position headroom under the asset limit
    """

    def __init__(self, products: tuple[str, ...] = DISABLE_PENNY_PRODUCTS,
                 size: int = DISABLE_PENNY_SIZE, limit: int = LIMIT) -> None:
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in self.products:
            ...
            # Improve by 1 tick when the spread allows; otherwise quote at BBO.
            if best_ask - best_bid >= 2:
                buy_px, sell_px = best_bid + 1, best_ask - 1
            else:
                buy_px, sell_px = best_bid, best_ask
            # Self-cross guard: in a 1-tick spread improving both sides
            # collapses to the same price; fall back to BBO.
            if buy_px >= sell_px:
                buy_px, sell_px = best_bid, best_ask
            ...
            orders: List[Order] = []
            buy_qty = min(self.size, buy_room)
            if buy_qty > 0:
                orders.append(Order(product, int(buy_px), int(buy_qty)))
            sell_qty = min(self.size, sell_room)
            if sell_qty > 0:
                orders.append(Order(product, int(sell_px), -int(sell_qty)))
...
class PebblesBasketBalancer:
    """
    Quote PEBBLES_M and PEBBLES_XS in the direction that reduces the
    PEBBLES basket residual.

    The basket invariant (sum of all 5 PEBBLES ~= 50,000) is computed by
    ``_pebbles_group_residual_and_fair``. When |residual| crosses
    ``DISABLE_BASKET_TRIGGER`` we quote one-sided:

      residual > 0  -> basket overvalued -> SELL the disabled members
      residual < 0  -> basket undervalued -> BUY the disabled members
      |residual| <= trigger -> stay flat (no new orders)

    Existing inventory can always be flattened back toward zero, even
    when the basket says "do not add new exposure", because we still
    quote the side that reduces the absolute position.
    """

    def __init__(self, products: tuple[str, ...] = DISABLE_BASKET_PRODUCTS,
                 size: int = DISABLE_PENNY_SIZE, limit: int = LIMIT,
                 trigger: float = DISABLE_BASKET_TRIGGER) -> None:
        self.products = products
        self.size = int(size)
        self.limit = int(limit)
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        residual, _ = _pebbles_group_residual_and_fair(state)
        out: Dict[str, List[Order]] = {}
        ...
        for product in self.products:
            ...
            # Same self-cross-aware quoting rule as the penny trader.
            if best_ask - best_bid >= 2:
                buy_px, sell_px = best_bid + 1, best_ask - 1
            else:
                buy_px, sell_px = best_bid, best_ask
            ...
            if residual is not None and abs(residual) > self.trigger:
                # Basket signal active: quote the corrective side.
                if residual > 0:
                    quote_sell = True   # need to push basket DOWN -> sell M/XS
                else:
                    quote_buy = True    # need to push basket UP -> buy M/XS

            # Always allow a flattening quote on the opposite side of an
            # existing position so inventory can mean-revert to zero.
            if position > 0:
                quote_sell = True
            elif position < 0:
                quote_buy = True
...
class Trader:
    def __init__(self, *args, **kwargs) -> None:
        ...
        # V13: cover the 6 previously-disabled products with conservative quoters.
        self.penny_disabled = PennyDisableTrader()
        self.pebbles_basket = PebblesBasketBalancer()
        # Set of products owned by the V13 coverage layer; we never let
        # base/V5 orders for these products through, so the conservative
        # quoters are the single source of truth for them.
        self._coverage_products: set[str] = set(DISABLE_PENNY_PRODUCTS) | set(DISABLE_BASKET_PRODUCTS)
    ...
    def run(self, state: TradingState):
        ...
        for product, orders in base_orders.items():
            if (product not in USE_V5_PRODUCTS
                    and product not in DISABLE_PRODUCTS
                    and product not in self._coverage_products):
        ...
        # V13 coverage layer: penny-MM for the 4 plain disabled, and a
        # basket-balancing quoter for the 2 disabled PEBBLES. These run
        # last so they always own their products in the final book.
        for product, orders in self.penny_disabled.run(state).items():
            result[product] = orders
        for product, orders in self.pebbles_basket.run(state).items():
            result[product] = orders
