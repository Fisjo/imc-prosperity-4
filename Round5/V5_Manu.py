...
MICROCHIP_PRODUCTS = ("MICROCHIP_CIRCLE", "MICROCHIP_OVAL", "MICROCHIP_RECTANGLE", "MICROCHIP_SQUARE", "MICROCHIP_TRIANGLE")
...
class MmSkewBaseTrader:
    def __init__(
        ...
        *,
        ...
        self.data_dir = _default_data_dir() if data_dir is None else str(data_dir)
...
def _load_day_prices(data_dir: str, day: int) -> DayPrices:
    path = f"{data_dir}/prices_round_{ROUND_NUM}_day_{day}.csv"
...
def _get_best_market(depth: OrderDepth):
    best_bid = _best_bid(depth)
    best_ask = _best_ask(depth)
...
def compute_microchip_fair(depth, mid, score, prior, pos, kalman_x):
    ...
    return (0.52 * mid) + (0.43 * micro) + (0.05 * kalman_x) + signal_shift + imbalance_shift - inventory_skew


def apply_micro_ols_adjustments(mids, adjs):
...
class V5PebblesAndStructureTrader:
    ...
    def trade_microchip_cluster(
        ...
        # Gather current mids and update per-product history/open price
        mids: Dict[str, float] = {}
        ...
        # Base OLS cluster scores
        scores: Dict[str, float] = {p: 0.0 for p in mids}
        scores = apply_micro_ols_adjustments(mids, scores)

        # Lead-lag adjustments
        for leader, follower, sign in MICRO_LEAD_LAG:
            if leader not in mids or follower not in mids:
                continue
            leader_hist = data.get("micro_hist_" + leader, [])
            if len(leader_hist) >= 2:
                leader_return = leader_hist[-1] - leader_hist[-2]
                scores[follower] = scores.get(follower, 0.0) + sign * leader_return
        ...
        for product, cfg in TREND_OVERLAY.items():
            ...
            hist = data.get("micro_hist_" + product, [])
            if len(hist) >= min_ticks:
                trend_val = hist[-1] - hist[-min_ticks]
                if abs(trend_val) >= threshold:
                    scores[product] = scores.get(product, 0.0) + (1.0 if trend_val > 0 else -1.0)
        ...
        for product, cfg in OPEN_MOM_OVERLAY.items():
            ...
            k = cfg["k"]
            th = cfg["th"]
            open_price = data.get("open_mid_" + product)
            if open_price is not None:
                open_move = mids[product] - open_price
                if abs(open_move) > th:
                    scores[product] = scores.get(product, 0.0) + open_move / k
        ...
        for product in MICROCHIP_PRODUCTS:
            ...
            fair = compute_microchip_fair(depth, m, score, prior, pos, 0.0)
            signal = fair - m
            ...
            if signal > ENTRY_EDGE:
                ...
                self.add_sell(state, orders_by_product, product, sell_px, LIMIT)
            elif abs(signal) < EXIT_EDGE and pos != 0:
                if pos > 0:
                    self.add_sell(state, orders_by_product, product, sell_px, pos)
                else:
                    self.add_buy(state, orders_by_product, product, buy_px, -pos)
