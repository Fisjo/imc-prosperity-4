...
class Trader:
    ...
    def trade_osmium(self, od: OrderDepth, pos: int) -> List[Order]:
        ...
        # NO CLEAR PHASE (the key change)
        # Instead, use position-aware making
        ...
        # Emergency clear only when near limits
        if abs(pos) >= 50:
            if pos > 0 and sell_cap > 0:
                vol = min(pos - 40, sell_cap)  # clear down to 40
                orders.append(Order(self.OSMIUM, FV, -vol))
                sell_cap -= vol
            elif pos < 0 and buy_cap > 0:
                vol = min(-pos - 40, buy_cap)  # clear up to -40
                orders.append(Order(self.OSMIUM, FV, vol))
                buy_cap -= vol
