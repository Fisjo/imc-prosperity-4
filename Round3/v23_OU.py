...
ITM_STRIKES = (4000, 4500)  # deep-ITM, treated as underlying-proxies (no BS)

# VFE long-run mean (cross-day estimate from R3 days 0-2: 5246.51, 5248.39, 5255.39
# combined mean = 5250.10). For deep-ITM options where d = (S-K)/(sigma*sqrt(T)) >> 0,
# Bachelier price collapses to S - K, so FV(VEV_K) = E[S] - K = VFE_ANCHOR - K.
# This structural prior beats within-day microprice estimation (cross-product OU inference).
VFE_ANCHOR_FOR_OPTIONS = 5250
ITM_STRUCTURAL_FV = {K: VFE_ANCHOR_FOR_OPTIONS - K for K in ITM_STRIKES}
...
class Trader:
    ...
    def vfe_scaled(self, depth, pos):
        """v9-style scaled threshold for VFE. Anchor=5250."""
    ...
    def hgp_scaled(self, depth, pos):
        """Their alt strategy: FAIR=9985, TAKE_EDGE=36, TAKE_CLIP=20, INVENTORY_SKEW=4."""
        sym = 'HYDROGEL_PACK'
        if not depth.buy_orders or not depth.sell_orders: return []
        FAIR = 9985
        TAKE_EDGE = 36
        ...
        # Take aggressively when book is far from fair
        buy_left = bc
        ...
        # Passive MM at fair with inventory skew
    ...
    def trade_itm_option(self, sym, depth, pos, out, K, take_edge=1, scale=19, passive_clip=50):
        """Fixed-FV market maker for deep-ITM options (delta~1, behave as underlying-proxy).
        Anchor uses STRUCTURAL PRIOR: FV = VFE_ANCHOR - K, derived from VFE's cross-day
        long-run mean. For deep-ITM, Bachelier price collapses to S - K, and E[S] is the
        OU/AR(1) long-run mean of VFE which we already pin at 5250 in the VFE strategy.
        Take/make logic mirrors VFE structure with thresholds at fair +/- 1 and inventory skew."""
        if not depth.buy_orders or not depth.sell_orders: return
        best_bid = max(depth.buy_orders)
        best_ask = min(depth.sell_orders)

        # Structural prior: FV = E[S] - K. No within-day estimation needed.
        fair = ITM_STRUCTURAL_FV[K]
        ...
        # Passive phase (only when not too leveraged)
        if abs(pos) < 150:
    ...
    def run(self, state):
        ...
        for K in DAVID_STRIKES:
            ...
            if sym in state.order_depths:
                pos = state.position.get(sym, 0)
                if K in ITM_STRIKES:
                    # Deep-ITM: structural-prior FV from VFE long-run mean (no Black-Scholes)
                    self.trade_itm_option(sym, state.order_depths[sym], pos, result, K=K)
                else:
                    # Mid-strike and OTM: Black-Scholes-style mispricing detector
